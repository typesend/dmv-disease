...
disease.renderers = {
  ...
  checkbox: function (el) {
    tagToRender = `<input id="${el.id}" name="${el.name}" type="checkbox" onclick="window.disease.click(this)" value="${el.value}" ${el.checked ? 'checked' : ''} ${el.disabled ? 'disabled' : ''}>`;
    return '<br/>' + tagToRender;
  },

  text: function (el) {
    tagToRender = `<input id="${el.id}" name="${el.name}" type="text" value="${el.value}" onchange="window.disease.typeInto(this)" size="${el.size}" ${el.disabled ? 'disabled' : ''}>`;
    return '<br/>' + tagToRender;
  },

  password: function (el) {
    tagToRender = `<input id="${el.id}" name="${el.name}" type="password" value="${el.value}" onchange="window.disease.typeInto(this)" size="${el.size}" ${el.disabled ? 'disabled' : ''}>`;
    return '<br/>' + tagToRender;
  },

  select: function (el) {
    tagToRender = `<select id="${el.id}" name="${el.name}" onchange="window.disease.selectOption(this)">`;
